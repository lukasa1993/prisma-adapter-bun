...
// Scalar PostgreSQL type OIDs we need (taken from PostgreSQL catalog)
...
// lightweight array parser – covers simple one-dimensional PG arrays
function basicParseArray<T>(input: string, transform: (v: string) => T): T[] {
  ...
  for (let i = 0; i < content.length; i++) {
    ...
    if (ch === "," && !inQuotes) {
      elements.push(transform(current));
      current = "";
    } else {
      current += ch;
    }
  }
  if (current.length) elements.push(transform(current));
  return elements;
}
...
function normalize_timestampz(time: string): string {
  return new Date(time.replace(/[+-]\d{2}(:\d{2})?$/, "Z"))
    .toISOString()
    .replace(/(\.000)?Z$/, "+00:00");
}
...
// Helper to parse Postgres bytea string (hex format starting with "\\x")
function parsePgBytes(str: string): Buffer {
  if (str.startsWith("\\x")) {
...
// BYTEA array parser using postgres-array
const parseBytesArray = (input: string) => basicParseArray(input, parsePgBytes);

function normalizeByteaArray(serializedBytesArray: string) {
...
function normalize_array(
  element_normalizer: (v: string) => string,
): (str: string) => string[] {
  return (str) => basicParseArray(str, element_normalizer);
}
